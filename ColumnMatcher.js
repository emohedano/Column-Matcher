	...
	function generaLista(name,urlData,conectors){
		...
		$.getJSON(urlData, function(data) {
			...
			$.each(data, function(key, val) {				
				...
				var idElement = 'data-id-element="'+key+'"';

				var colorElement = 'data-color-element="'+((val.indexOf("#")>-1)?val:null)+'"';

				

				if(conectors.indexOf('*')>-1){

					element = '<li id="'+key+'" '+colorElement+'>'+
								'<div class="top" '+idElement+'></div>'+
								'<div class="inline left" '+idElement+'></div>'+
								'<div class="inline center">'+key+'</div>'+
					...
					element += '<div class="inline center">'+key+'</div>';
	...
	function clickOnEndpoint(element){
		...
		var idSelectedElement = null;
				...
				if( selectedElement == null){
					...
					if(connectorColor == null || connectorColor == "null")
